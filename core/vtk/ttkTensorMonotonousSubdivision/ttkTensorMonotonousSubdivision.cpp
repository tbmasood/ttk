...
int ttkTensorMonotonousSubdivision::FillInputPortInformation(
...
int ttkTensorMonotonousSubdivision::FillOutputPortInformation(
  int port, vtkInformation *info) {
  switch(port) {
    case 0:
...
vtkSmartPointer<vtkDataArray>
  ttkTensorMonotonousSubdivision::AllocateField(vtkDataArray *const inputField,
                                                int ntuples) const {
  ...
  switch(inputField->GetDataType()) {
    ...
    default:
      ...
      msg << MODULE_TMS "Unsupported data array type" << endl;
...
int ttkTensorMonotonousSubdivision::InterpolateFields(
  vtkDataSet *const input, vtkUnstructuredGrid *const output) const {
  ...
  for(size_t i = 0; i < ncelldata; ++i) {
    ...
    switch(inputField->GetDataType()) {
      vtkTemplateMacro(baseWorker_.interpolateCellDataField<VTK_TT>(
        static_cast<VTK_TT *>(inputField->GetVoidPointer(0)),
        static_cast<VTK_TT *>(outputField->GetVoidPointer(0)),
        inputField->GetNumberOfComponents()));
    }
...
int ttkTensorMonotonousSubdivision::doIt(std::vector<vtkDataSet *> &inputs,
  ...
  switch(input->GetPointData()->GetTensors()->GetDataType()) {
    ...
    default:
      std::stringstream msg;
      msg << MODULE_TMS "Unsupported data type :(" << endl;
  ...
  {
    std::stringstream msg;
    msg << MODULE_TMS "Memory usage: " << m.getElapsedUsage() << " MB." << endl;
